...
class DesktopPet(QWidget):
    ...
    def show_context_menu(self, position):
        ...
        chat_action.triggered.connect(self._on_chat_clicked)  # 改成呼叫方法 !!!!!!!!!!!!!!!!!!!
    ...
    def _on_chat_clicked(self):
        if not self.on_chat_request:
            QMessageBox.warning(self, "尚未設定", "尚未提供對話回呼 on_chat_request。")
            return
        try:
            self.on_chat_request()
        except Exception as e:
            import traceback
            tb = traceback.format_exc()
            QMessageBox.critical(self, "對話啟動失敗", f"{e}\n\n{tb[:1200]}")
